...
class DiceBox {
  constructor(element_container, options = {}) {
    ...
    if (options.presets) {
      for (let pname in options.presets) {
        DicePreset.addPreset(pname, options.presets[pname]);
      }
    }
  }
  ...
  async loadTheme(themeConfig) {
    ...
    console.log("Loaded colorset:", colorData);
    this.DiceFactory.applyColorSet(colorData, true);
