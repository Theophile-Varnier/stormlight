...
export class DicePreset {
  ...
  static addPreset(name, config) {
    DicePreset.dice[name] = config;
  }
  constructor(name) {
    if (!DicePreset.dice.hasOwnProperty(name)) {
      return console.error("dice type unavailable");
    }
    Object.assign(this, defaults, DicePreset.dice[name]);
    this.shape = DicePreset.dice[name].type || name;
    this.type = name;
