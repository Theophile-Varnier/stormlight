...
class DiceFactory {
  ...
  create(type) {
    ...
    const colorset = diceobj.colorset
      ? COLORSETS[diceobj.colorset]
      : this.default_colordata;
  ...
  applyColorSet(colordata, isDefault = false) {
    this.colordata = colordata;
    if (isDefault) {
      this.default_colordata = colordata;
    }
  ...
  setMaterialInfo(colorset = null) {
    let prevcolordata = colorset || this.colordata;
    let prevtexture = this.dice_texture;
    let prevmaterial = this.dice_material;

    if (this.colordata && this.colordata.name != prevcolordata.name) {
